...
`describeElement`/`ElementResolver`. Автор сохраняет descriptors и area keys в отдельный документ
...
При выборе состава во время записи `RecordingSessionService` приостанавливает recorder до переноса фокуса и возобновляет
после закрытия слоя. Core `pause/resume` сохраняет журнал и отменяет неподтверждённые действия, а также защищает от
запоздалых microtasks. Изменения DOM во время паузы учитываются при возобновлении как состояние, не как действия.
Собственного механизма выбора/подсветки в библиотеке нет; приложение управляет причиной и длительностью паузы.
