        ...
        .evaluateAll((rows) =>
            Object.fromEntries(
                rows.map((row) => [
                    ...
                    Number(row.getAttribute('data-scans')),
                ]),
            ),
        );
}
...
for (const {count, mixed} of [
    {count: 10, mixed: true},
    {count: 100, mixed: false},
    {count: 300, mixed: false},
]) {
    test(`${count} dynamic areas retain isolated updates and release replaced roots`, async ({
        page,
    }, testInfo) => {
        ...
        report['synchronousStartMs'] = Number(
            (await page.getByTestId('load-start-ms').textContent())!.replaceAll(',', ''),
        );
        await page.waitForTimeout(800);

        let before = await counters(page);

        report['idleWithoutPolling'] = await measure(client, async () =>
        ...
        report['idleWithPolling'] = await measure(client, async () =>
            page.waitForTimeout(1600),
        );
        expect(await counters(page)).toEqual(before);
        ...
        report['singleAreaBurst'] = await measure(client, async () => {
            await page.locator('#load-1-0').evaluate((input: HTMLInputElement) => {
                ...
                for (let i = 0; i < 200; i++) {
                    input.setAttribute('data-version', String(i));
                }

                input.dispatchEvent(new Event('input', {bubbles: true}));
            });
            await expect(
                page.locator('.area-status[data-name="load-1"]'),
        ...
        if (mixed) {
            ...
            report['largeAreaUpdate'] = await measure(client, async () => {
                ...
                await expect(
                    page.locator('.area-status[data-name="load-6"]'),
                ).toHaveAttribute('data-value', 'Большая форма');
            });
            await page.waitForTimeout(800);
        ...
        report['continuousSingleArea'] = await measure(client, async () => {
            ...
            await expect(
                page.locator('.area-status[data-name="load-1"]'),
            ).toHaveAttribute('data-value', 'tick-20');
        });
        await page.waitForTimeout(800);
        ...
        report['allAreasBurst'] = await measure(client, async () => {
            ...
            await expect(page.locator('.area-status[data-value="Массовое"]')).toHaveCount(
                count,
            );
        });
        await page.waitForTimeout(800);
        expect(await counters(page)).toEqual(
            Object.fromEntries(
                Object.entries(before).map(([name, scans]) => [name, scans + 1]),
            ),
        );
        ...
        report['replaceRoots'] = await measure(client, async () => {
            await page
                .getByRole('button', {name: 'Заменить первые 30', exact: true})
                .click();
            await expect(
                page.locator(
                    `.area-status[data-name="load-${count + replacementCount}"]`,
                ),
            ).toHaveCount(1);
            await expect(page.getByTestId('load-count')).toHaveText(String(count));
        });
        ...
        for (let id = 1; id <= replacementCount; id++) {
            expect(replaced[`load-${id}`]).toBeUndefined();
        }

        for (let id = replacementCount + 1; id <= count; id++) {
            expect(replaced[`load-${id}`]).toBe(before[`load-${id}`]);
        }
        ...
        report['stopped'] = await measure(client, async () => {
            await page.locator('[data-mf] input').evaluateAll((inputs) => {
                for (const input of inputs) {
                    (input as HTMLInputElement).value = 'После stop';
                }
            });
...
test('CPU profile of simultaneous updates in 300 areas', async ({page}, testInfo) => {
    ...
    for (const [index, id] of (profile.samples ?? []).entries()) {
        ...
        const urls = frame.url.split('/');
        const name = `${frame.functionName || '(anonymous)'} ${urls[urls.length - 1] ?? ''}:${frame.lineNumber + 1}`;

        sampled.set(name, (sampled.get(name) ?? 0) + (profile.timeDeltas?.[index] ?? 0));
    }

    const hottest = [...sampled]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 20)
        .map(([frame, microseconds]) => ({
            frame,
            sampledMs: Math.round(microseconds / 1000),
        }));

    await saveArtifact(testInfo, 'cpu-summary.json', hottest);
    console.info(JSON.stringify({profile: hottest}));
});
