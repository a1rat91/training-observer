...
конфигураций генераторов; можно проверить CSS на текущем DOM, включить подсветку Driver.js и диагностическую запись
rrweb. Кнопка «Начать запись» сохраняет начальные DOM-значения и включает журнал изменений (до 50 записей). В журнале
видны строки, пустые значения, checked для checkbox/switch и источник: snapshot/native/rrweb. Для полей демоформы rrweb
сохраняет реальные значения, остальные input маскируются, password скрывается. Маскирование до внутреннего dedup rrweb
иначе подавляет программные замены строк одинаковой длины — это покрыто browser test. DOM-строка Taiga Number/Date и
отображаемое имя Select не равны гарантированно доменному значению Angular/backend. Запись останавливается кнопкой и при
переходе на другую страницу. Это панель исследования библиотек; плеер процедур и семантический recorder будут отдельными
этапами.
