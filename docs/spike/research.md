...
## Статус после завершения spike

Ниже сохранены решения и измерения **этапа 2**, не текущий состав MVP. Итоговое решение находится в
[architecture.md](./architecture.md), границы — в [limitations.md](./limitations.md). В recorder v2 используются
собственная observation-логика и typed locators; dom-to-locator остался comparator, rrweb — отдельной диагностикой.
Driver.js исключён по решению пользователя на этапе 9: зависимость, CSS, кнопка подсветки и исполняемый probe удалены. В
сохранённом `library-probes.json` его прежние результаты остаются историческими; нынешняя команда `research:spike`
выполняется без Driver.js. Исходники прежнего runner доступны в коммите `aac891b`. Повторный запуск не обещает
воспроизвести прежние rrweb event counts/размеры побайтно.
...
## Интерактивная страница в demo

После `npm start` откройте [localhost:4200/spike/research](http://localhost:4200/spike/research) или пункт меню **Spike
→ Исследование locators**. Корневой адрес также перенаправляет сюда.

Слева — рабочая форма на Taiga UI 4. Справа после клика на контрол показаны accessible name/role и результаты четырёх
конфигураций генераторов; можно проверить CSS на текущем DOM и включить диагностическую запись rrweb. Кнопка «Начать
запись» сохраняет начальные DOM-значения и включает журнал изменений (до 50 записей). В журнале видны строки, пустые
значения, checked для checkbox/switch и источник: snapshot/native/rrweb. Для полей демоформы rrweb сохраняет реальные
значения, остальные input маскируются, password скрывается. Маскирование до внутреннего dedup rrweb иначе подавляет
программные замены строк одинаковой длины — это покрыто browser test. DOM-строка Taiga Number/Date и отображаемое имя
Select не равны гарантированно доменному значению Angular/backend. Запись останавливается кнопкой и при переходе на
другую страницу. Это панель исследования библиотек; плеер процедур и семантический recorder будут отдельными этапами.

Ниже находится сохранённая таблица 504 сравнений и кнопка скачивания JSON. Отчёт импортируется из
`docs/spike/library-probes.json` при сборке; он не пересчитывается кликами на странице. `npm run research:spike` создаёт
новые результаты в `dist/spike-research`, а обновление зафиксированного отчёта делается отдельно после проверки.

Маршрут проверяется командой `npx playwright test tests/research.spec.ts`: Taiga Select, генерация locators, подсветка,
rrweb, значения text/textarea/checkbox/switch/Select, программная замена value без native events, остановка/перезапуск,
скачивание 504 строк и уход/возврат через Angular Router.
