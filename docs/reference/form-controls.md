...
добавлены `radio`, `switch` и `number`; версия Taiga UI — 5.15.0.
...
`kind: 'number'` отличает числовое поле от общего textbox. Значение `state.value`
остаётся **строкой DOM без преобразований**: сохраняются разделители, префикс,
постфикс, знак, пустая строка и промежуточный ввод. Например, `1 250,5 ₽` не
превращается в `1250.5`. Это предотвращает угадывание формата и потерю точности
для bigint. `inputmode="numeric"` сам по себе не делает поле числовым: так могут
вводиться номера договоров с ведущими нулями. ARIA spinbutton без native input
и числовые поля внутри `tui-textfield[multi]` пока не получают этот адаптер.

`locatorHints.role` сохраняет фактическую роль: masked input Taiga без явной роли
остаётся textbox, обычный input number имеет implicit role spinbutton. Тип
контрола и роль в accessibility tree — разные признаки.

Поле получает host своего `tui-textfield` и связанные подписи. Cleaner входит
в `memberNodeIds`, как у текстовых полей. Кнопки `+`/`−` остаются отдельными
логическими кнопками с собственными label и disabled: нажатие и результат
изменения числа впоследствии нужно будет различать при записи действий.
Их label не является уникальным локатором при нескольких числовых полях.
Явно связанный через ARIA dropdown (`aria-haspopup`, `aria-expanded`,
`aria-controls`) попадает в `popup`, без объявления его
списком выбора или доказательством выбранного значения.

Обычный ввод обнаруживается через события, изменения value без событий — через
сверку native-свойств. Это относится и к обновлениям из Angular, включая кнопки
шага, если конкретный компонент не отправляет native input/change. При отключении
сверки такие изменения без мутаций потребуют refresh. Scroll/hover не добавлены
в источники событий.

В Taiga 5.15 настройки `[min]`, `[max]`, `[step]`, number format и value transformer
могут оставаться в директивах и DI, без отражения в HTML-атрибутах. Парсер не
читает Angular-модель и не обещает восстановить эти ограничения либо значение
после transformer. Атрибуты min/max/step, если присутствуют, доступны в исходном
DOM-снимке; отдельной модели числовых ограничений пока нет. `invalid` отражает
только наблюдаемую браузерную/ARIA-валидацию, а не все ошибки Angular FormControl.
Для проверки ученика семантическое сравнение `1 000,00` и `1000` ещё предстоит
реализовать; на текущем шаге библиотека сохраняет наблюдаемый текст поля.

В `/controls` раскрыть «InputNumber». Стенд содержит реальные поля Taiga с валютой,
другими разделителями, bigint за пределами Number.MAX_SAFE_INTEGER и обычный HTML
input number. `tests/number-controls.spec.ts` проверяет точность строки, подписи,
ввод/шаг/очистку, доступность кнопок на границах, native-валидацию, изменения
свойств без событий, распознавание и явно связанный popup.

Следующие семейства: Slider/Range, дата/время, поля с несколькими
