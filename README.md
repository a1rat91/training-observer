...
поиском, динамически открываемым плеером и соседними MF. Выбор областей ещё не реализован.
...
Раздел «Эксперименты с микрофронтами» позволяет переставлять соседей и пересоздавать плеер. На /spike/learn тот же
стенд; ручной запуск обучения пока диагностический, свободный порядок и blur-only ещё запланированы.
