...
lifecycle, portals и оптимизация; блокировка взаимодействий выделена в отдельное расширение. Добавлен первый стенд с
поиском, динамически открываемым плеером и соседними MF. Registry границ добавлен; выбор областей ещё не подключён к
записи.
...
На [странице записи](http://localhost:4200/spike/record) начните запись, затем выберите «Заявка на обучение» в поиске:
плеер откроется ниже автоматически. Поиск, плеер, справочник и уведомления — отдельные DOM-hosts в одном Angular demo.
Раздел «Эксперименты с микрофронтами» позволяет переставлять соседей и пересоздавать плеер. В панели записи раскройте
«Границы микрофронтов», чтобы увидеть статусы и поколения областей. На /spike/learn тот же стенд; ручной запуск обучения
пока диагностический, свободный порядок и blur-only ещё запланированы.
