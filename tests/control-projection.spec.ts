...
import {type DomElementSnapshot, type DomNodeSnapshot, type DomSnapshot} from '../libs/training-observer/src/lib/models/dom-snapshot';
import {ControlSnapshotBuilder} from '../libs/training-observer/src/lib/services/control-snapshot-builder';

function element(id: string, tagName: string, attributes: Record<string, string> = {},
    children: string[] = [], overrides: Partial<DomElementSnapshot> = {}): DomElementSnapshot {
    return {
        kind: 'element', id, parentId: null, tagName, attributes, children, path: `/${id}`, label: '',
        rects: [], visible: true, inViewport: true, hitTest: 'hit', interactive: true,
        interactionReasons: ['native'], pointerActionable: true, boundaries: [],
        state: {disabled: false, readOnly: false, inert: false, required: false, invalid: false, redacted: false},
        ...overrides,
    };
}
...
function snapshot(...items: DomNodeSnapshot[]): DomSnapshot {
    const nodes: Record<string, DomNodeSnapshot> = Object.fromEntries(items.map((node) => [node.id, node]));
    for (const node of items) {
        if (node.kind !== 'element') continue;
        for (const child of node.children) {
            if (nodes[child]) nodes[child] = {...nodes[child], parentId: node.id};
        }
    }

    return {
        schemaVersion: 1, capturedAt: '2026-09-10T00:00:00.000Z', durationMs: 0,
        rootId: items[0]?.id ?? null, relatedRootIds: [], nodes, interactiveIds: [],
        stats: {nodeCount: items.length, elementCount: items.filter((node) => node.kind === 'element').length,
...
test('projection works on a saved snapshot without a document or Angular injector and does not mutate it', () => {
    ...
    const saved = snapshot(element('field', 'tui-textfield', {}, ['label', 'number']),
        element('label', 'label', {tuilabel: ''}, ['caption']), text('caption', '  Сумма  '),
    ...
    expect(result[0]).toMatchObject({kind: 'number', source: 'taiga-ui', label: 'Сумма',
        hostNodeId: 'field', targetNodeId: 'number', state: {value: '9 007 199 254 740 993 ₽'}});
...
test('Taiga priority and intentional exclusions survive native fallback', () => {
    ...
    const result = new ControlSnapshotBuilder().build(snapshot(
        element('root', 'section', {}, ['select', 'unknown-select', 'multi', 'option', 'field']),
    ...
    expect(result.map((control) => [control.targetNodeId, control.kind, control.source]))
...
test('members retain cleaner and label order but stop at independent controls', () => {
    const saved = snapshot(
        ...
        element('label', 'label', {tuilabel: ''}, ['caption']), text('caption', 'Имя'),
        element('input', 'input', {tuiinput: ''}),
        element('cleaner', 'button', {tuibuttonx: ''}, ['clear-caption']), text('clear-caption', 'Очистить'),
        element('button', 'button', {}, ['button-caption']), text('button-caption', 'Проверить'),
    );
    const result = new ControlSnapshotBuilder().build(saved);

    expect(result.map((control) => control.targetNodeId)).toEqual(['input', 'button']);
    expect(result[0].memberNodeIds).toEqual(['field', 'label', 'caption', 'input', 'cleaner', 'clear-caption']);
...
test('ambiguous floating labels stay empty; an explicit captured label takes precedence', () => {
    const saved = snapshot(
        ...
        element('a', 'label', {tuilabel: ''}, ['a-text']), text('a-text', 'Первое'),
        element('b', 'label', {tuilabel: ''}, ['b-text']), text('b-text', 'Второе'),
        element('input', 'input', {tuiinput: ''}),
    );
    const builder = new ControlSnapshotBuilder();
    expect(builder.build(saved)[0].label).toBe('');
    const labelled = {...saved, nodes: {...saved.nodes, input: {...saved.nodes['input'], label: 'Явная подпись'}}};
...
test('duplicate HTML ids retain the existing distinct context and popup policies', () => {
    const saved = snapshot(
        element('group', 'section', {'aria-labelledby': 'duplicate'}, ['a', 'b', 'input']),
        element('a', 'div', {id: 'duplicate'}, ['a-text']), text('a-text', 'Первое'),
        element('b', 'div', {id: 'duplicate'}, ['b-text']), text('b-text', 'Последнее'),
        element('input', 'input', {tuiinput: '', role: 'combobox', 'aria-expanded': 'true', 'aria-controls': 'duplicate'}),
    );
    const control = new ControlSnapshotBuilder().build(saved)[0];

    expect(control.locatorHints.context).toEqual([{tagName: 'section', id: undefined, label: 'Последнее'}]);
...
test('truncated graph references are not invented and redacted values are not projected', () => {
    ...
    const saved = snapshot(element('field', 'tui-textfield', {}, ['secret', 'missing']),
        {...input, state: {...input.state, redacted: true, value: 'must-not-leak'}});
    const truncated: DomSnapshot = {...saved, stats: {...saved.stats, truncated: true, limitsReached: ['maxNodes']}};
