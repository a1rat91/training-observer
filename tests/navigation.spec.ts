...
test('demo menu follows lazy routes, browser history and direct links', async ({
    page,
}) => {
    await page.goto('/');
    ...
    await expect(menu.getByRole('link', {name: 'Контролы', exact: true})).toHaveAttribute(
        'aria-current',
        'page',
    );
    ...
    await expect(
        menu.getByRole('link', {name: 'Микрофронты', exact: true}),
    ).toHaveAttribute('aria-current', 'page');
    await expect(
        menu.getByRole('link', {name: 'Контролы', exact: true}),
    ...
    await expect(menu.getByRole('link', {name: 'Нагрузка', exact: true})).toHaveAttribute(
        'aria-current',
        'page',
    );
