...
test('real Taiga Radio and Switch retain individual labels and native states', async ({page}) => {
    await start(page);
    expect(await control(page, 'delivery-email')).toMatchObject({kind: 'radio', source: 'taiga-ui',
        label: 'Электронная почта', state: {checked: true, value: 'email', required: true},
        locatorHints: {role: 'radio', name: 'delivery', context: expect.arrayContaining([
            {tagName: 'fieldset', id: 'delivery-group', label: 'Способ доставки'},
        ])}});
    expect(await control(page, 'delivery-sms')).toMatchObject({kind: 'radio', label: 'SMS', state: {checked: false, value: 'sms'}});
    expect(await control(page, 'delivery-paper')).toMatchObject({kind: 'radio', label: 'Бумажное письмо',
        state: {disabled: true, checked: false}, pointerActionable: false});
    expect(await control(page, 'auto-save')).toMatchObject({kind: 'switch', source: 'taiga-ui',
        label: 'Автосохранение', state: {checked: false}, locatorHints: {role: 'switch'}});
...
test('radio and switch labels resolve for, wrapping labels and ARIA without using the group legend', async ({page}) => {
    await start(page);
    ...
    expect(await control(page, 'switch-aria')).toMatchObject({kind: 'switch', source: 'native', label: 'Переключатель с ARIA'});
    expect((await control(page, 'switch-wrapped'))?.label).toBe('Переключатель с label');
    await page.locator('#switch-label').evaluate((node) => { node.textContent = 'Обновлённая подпись'; });
...
test('radio and switch inherit availability; a switch never becomes tri-state', async ({page}) => {
    ...
    await page.locator('#disabled-switch').evaluate((input: HTMLInputElement) => { input.indeterminate = true; });
...
test('a radio group split across microfrontends updates peers and resets through an external form without polling', async ({page}) => {
    ...
    await page.evaluate(() => {
        document.body.insertAdjacentHTML('beforeend', '<form id="shared-form"></form><form id="other-form"></form>');
        document.getElementById('mf-profile')!.insertAdjacentHTML('beforeend',
            '<label><input id="peer-a" type="radio" name="plan" form="shared-form" value="basic" checked required>Базовый</label>');
        document.getElementById('mf-employment')!.insertAdjacentHTML('beforeend',
            '<label><input id="peer-b" type="radio" name="plan" form="shared-form" value="pro" required>Расширенный</label>');
        document.getElementById('mf-address')!.insertAdjacentHTML('beforeend',
            '<label><input id="peer-other" type="radio" name="plan" form="other-form" checked>Другая форма</label>');
    });
    const areas = async (): Promise<MicrofrontendSnapshot[]> => JSON.parse(await page.getByTestId('mf-json').textContent() ?? '[]');
    const field = async (id: string) => (await areas()).flatMap((area) => area.logicalControls).find((item) => item.locatorHints.id === id);
