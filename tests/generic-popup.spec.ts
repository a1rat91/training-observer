...
import {type ControlSnapshot} from '../projects/training-observer/src/index';

test.use({trace: 'off'});

async function control(page: Page, id: string): Promise<ControlSnapshot | undefined> {
    const controls: ControlSnapshot[] = JSON.parse(await page.getByTestId('controls-json').textContent() ?? '[]');
...
test('real generic TuiInput resolves its popup without claiming selection semantics', async ({page}) => {
    ...
    await expect.poll(() => control(page, 'generic-hint')).toMatchObject({
        kind: 'textbox', source: 'taiga-ui', state: {value: 'Новый запрос'},
...
test('a generic input without an explicit link remains present with an unresolved popup', async ({page}) => {
    ...
    await expect.poll(() => control(page, 'unlinked-input')).toMatchObject({kind: 'textbox',
        state: {value: 'Запрос'}, popup: {status: 'unresolved', relation: 'missing', rootNodeIds: []}});
});

test('real dialog popup is captured outside the scope and reconciles silent input changes', async ({page}) => {
    ...
    await page.locator('#popup-detail').evaluate((input: HTMLInputElement) => { input.value = 'Без события'; });
    ...
    await expect.poll(async () => (await control(page, 'generic-dialog'))?.popup?.status).toBe('open');
    await page.locator('#popup-detail').evaluate((input: HTMLInputElement) => { input.value = 'После открытия'; });
...
test('an explicit link on the textfield host resolves a non-list popup', async ({page}) => {
    ...
    await expect.poll(async () => (await control(page, 'host-input'))?.popup).toMatchObject({
        status: 'open', relation: 'aria-controls', referencedIds: ['host-popup'], text: 'Внешнее действие',
    });
});
...
test('excluded portal properties do not trigger scans', async ({page}) => {
    await page.locator('#generic-dialog').focus();
    await expect.poll(async () => (await control(page, 'generic-dialog'))?.popup?.status).toBe('open');
    await page.locator('tui-dropdown').evaluate((popup) => popup.setAttribute('data-training-observer-ignore', ''));
    ...
    await page.locator('#popup-detail').evaluate((input: HTMLInputElement) => { input.value = 'Исключён'; });
...
test('native option label follows the visible label attribute', async ({page}) => {
    ...
    await expect.poll(async () => (await control(page, 'label-probe'))?.choice?.displayValue).toBe('Отображаемое имя');
