...
test('input confirmation stays unchanged while editing and accepts clearing only after blur', async ({page}) => {
...
test('compact Taiga form exposes snapshot cards and real forward/back/wrong transitions', async ({page}) => {
    const errors: string[] = [];
    ...
    await page.locator('#surname').fill('Смирнова');
    await expect(panel.getByRole('article', {name: 'Input · Фамилия', exact: true})).toContainText('Ожидает выхода из поля');
    ...
    await expect(panel.getByRole('article', {name: 'Select · Подразделение', exact: true})).toContainText('Ожидает выхода из поля');
    ...
    await expect(panel.getByRole('article', {name: 'ComboBox · Сотрудник', exact: true})).toContainText('Ожидает выхода из поля');
    await page.locator('#surname').focus();
