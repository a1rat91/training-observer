...
test('failed navigation can retry the requested screen without inventing a visit', async ({
    page,
}) => {
    await page.goto('/record');
    ...
    await page.route(
        '**/assets/procedure/screens.json',
        async (route) => route.fulfill({status: 500}),
        {times: 1},
    );
...
test('compact Taiga form exposes snapshot cards and real forward/back/wrong transitions', async ({
    ...
    await page.locator('#surname').fill('Смирнова');
    await expect(
        panel.getByRole('article', {name: 'Input · Фамилия', exact: true}),
    ).toContainText('Ожидает выхода из поля');
    await page.locator('#surname').press('Tab');
    await expect(
        panel.getByRole('article', {name: 'Input · Фамилия', exact: true}),
    ).toContainText('Смирнова');
    await page.locator('#department').click();
    await page.getByRole('option', {name: 'Поддержка', exact: true}).click();
    await expect(
        panel.getByRole('article', {name: 'Select · Подразделение', exact: true}),
    ).toContainText('Ожидает выхода из поля');
    await page.locator('#surname').focus();
    await expect(
        panel.getByRole('article', {name: 'Select · Подразделение', exact: true}),
    ).toContainText('Поддержка');
    await page.locator('#employee').fill('Анна');
    await page.getByRole('option', {name: 'Анна Смирнова', exact: true}).click();
    await expect(
        panel.getByRole('article', {name: 'ComboBox · Сотрудник', exact: true}),
    ).toContainText('Ожидает выхода из поля');
    await page.locator('#surname').focus();
    await expect(
        panel.getByRole('article', {name: 'ComboBox · Сотрудник', exact: true}),
    ).toContainText('Анна Смирнова');
...
test('HTTP loading is visible to the observer and inspector does not observe itself', async ({
    page,
}) => {
    await page.goto('/record');
    const panel = page.getByRole('complementary', {name: 'Распознанный экран'});
    ...
    await expect(panel).toContainText('Последнее готовое посещение: 2');
    await expect(
        panel.getByRole('article').filter({hasText: 'Показать в форме'}),
    ).toHaveCount(9);
    await page.screenshot({
        path: test.info().outputPath('screen-cards.png'),
        fullPage: true,
    });
});
