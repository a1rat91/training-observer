...
test('normalizes real Taiga UI textfield, cleaner, button and checkbox', async ({page}) => {
    ...
    const cleaner = Object.values(dom.nodes).find((node) => node.kind === 'element' && 'tuibuttonx' in node.attributes);
    ...
    expect((await controls(page)).find((item) => item.label === 'Сохранить'))
        .toMatchObject({kind: 'button', source: 'taiga-ui', state: {disabled: true}, pointerActionable: false});
    expect(await control(page, 'notifications')).toMatchObject({kind: 'checkbox', source: 'taiga-ui',
        label: 'Получать уведомления', state: {checked: true, indeterminate: false}});

    await page.locator('tui-textfield').filter({has: page.locator('#full-name')}).locator('[tuiButtonX]').click();
...
test('projects native controls with labels and search context but leaves unsupported controls in raw DOM', async ({page}) => {
    ...
    expect(await control(page, 'native-name')).toMatchObject({source: 'native', kind: 'textbox',
        state: {value: 'Анна', readOnly: true, required: true},
        locatorHints: {name: 'recipient', testId: 'recipient-field', placeholder: 'ФИО',
            context: expect.arrayContaining([{tagName: 'fieldset', id: 'billing', label: 'Платёжные данные'}])}});
    expect(await control(page, 'native-check')).toMatchObject({kind: 'checkbox', source: 'native', label: 'Согласие'});
...
test('keeps logical controls live for input, checkbox properties and availability changes', async ({page}) => {
    ...
    await page.locator('#full-name').evaluate((input: HTMLInputElement) => { input.disabled = true; });
...
test('preserves hints across value changes and DOM replacement without pretending IDs are stable', async ({page}) => {
    await page.getByTestId('observed-page').evaluate((root) => root.insertAdjacentHTML('beforeend',
...
test('does not merge independent buttons or fields just because their host or label matches', async ({page}) => {
    await page.locator('tui-textfield').filter({has: page.locator('#full-name')}).evaluate((host) => host.insertAdjacentHTML('beforeend',
    ...
    const filler = Object.values((await snapshot(page)).nodes)
        .find((node) => node.kind === 'element' && node.attributes['id'] === 'field-filler');
...
test('all control references resolve in the same snapshot, including truncated captures', async ({page}) => {
    ...
    for (const item of logical) {
        for (const id of [item.targetNodeId, item.hostNodeId, ...item.memberNodeIds]) expect(dom.nodes[id]).toBeDefined();
    }
});
