...
async function element(page: Page, id: string): Promise<DomElementSnapshot | undefined> {
    return Object.values((await snapshot(page)).nodes).find(
        (node): node is DomElementSnapshot => node.kind === 'element' && node.attributes['id'] === id,
    );
}
...
function reads(page: Page): Promise<number> {
    return page.evaluate(
        () =>
            (document.getElementById('read-probe') as HTMLInputElement & { testReadCount: number })
                .testReadCount,
    );
}
...
test('batches a burst once and publishes nothing for an unchanged recomputation', async ({ page }) => {
    ...
    await expect
        .poll(async () => (await element(page, 'batch-probe'))?.attributes['data-sequence'])
        .toBe('99');
...
test('does not starve updates during a continuous stream of changes', async ({ page }) => {
    ...
    try {
        ...
        await expect
            .poll(async () => Number((await element(page, 'full-name'))?.attributes['data-tick'] ?? 0), {
                intervals: [30],
                timeout: 2000,
            })
            .toBeGreaterThan(0);
...
test('plain scroll does not rebuild or publish a snapshot, including with property polling enabled', async ({
    ...
    expect(await page.locator('#passive-row').evaluate((row) => row.getBoundingClientRect().y)).toBe(
        oldY - 150,
    );
...
test('DOM changes caused by virtualized scrolling still update the snapshot with polling disabled', async ({
    ...
    await page.getByTestId('observed-page').evaluate((root) => {
        ...
        document.getElementById('virtual-scroller')!.addEventListener(
            'scroll',
            () => {
...
test('decorative hover on a real Taiga UI button does not rebuild the snapshot', async ({ page }) => {
    ...
    const transitions = await button.evaluate(
        (element) => (element as HTMLElement & { hoverTransitions: string[] }).hoverTransitions,
    );
...
test('menus inserted and removed on hover still update the page context', async ({ page }) => {
    ...
    await page.getByTestId('observed-page').evaluate((root) => {
        ...
        host.addEventListener('mouseenter', () =>
            host.insertAdjacentHTML('beforeend', '<button id="hover-menu-action">Открыть</button>'),
        );
    ...
    test(`real wheel scrolling with Taiga UI page scrollbars does not trigger a capture (wholeDocument=${wholeDocument})`, async ({
        ...
        expect(
            Object.values((await snapshot(page)).nodes).some(
                (node) =>
                    node.kind === 'element' &&
                    (node.tagName === 'tui-scroll-controls' || 'tuiscrollbar' in node.attributes),
            ),
        ).toBe(false);
        ...
        await expect
            .poll(async () => (await element(page, 'full-name'))?.attributes['data-scroll-content'])
            .toBe('updated');
    });
}
