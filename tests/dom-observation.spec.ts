...
async function settings(page: Page, poll = 0, batch = 50): Promise<void> {
    ...
    await expect.poll(async () => count(page)).toBeGreaterThan(before);
...
async function installReadProbe(page: Page): Promise<void> {
    ...
    await expect
        .poll(async () => (await element(page, 'read-probe'))?.tagName)
        .toBe('input');
}

async function reads(page: Page): Promise<number> {
    return page.evaluate(() => document.querySelector('#read-probe')!.testReadCount);
}
...
test('updates text, attributes, additions and removals without manual capture', async ({
    page,
}) => {
    await settings(page);
    await page.getByTestId('observed-page').evaluate((root) => {
    ...
    await expect
        .poll(async () => (await element(page, 'live-button'))?.state.disabled)
        .toBe(true);
    expect((await element(page, 'live-button'))?.label).toBe('После');
    await page.getByRole('button', {name: 'Добавить поле'}).click();
    await expect
        .poll(async () => (await element(page, 'comment'))?.tagName)
        .toBe('textarea');
    await page.getByRole('button', {name: 'Убрать поле'}).click();
    await expect.poll(async () => element(page, 'comment')).toBeUndefined();
    await page.locator('#live-button').evaluate((button) => button.remove());
    await expect.poll(async () => element(page, 'live-button')).toBeUndefined();
});

test('capture-phase input/change work with polling disabled and stopped bubbling', async ({
    page,
}) => {
    await settings(page);
    ...
    await expect
        .poll(async () => (await element(page, 'full-name'))?.state.value)
        .toBe('Событие ввода');
    ...
    await expect
        .poll(async () => (await element(page, 'notifications'))?.state.checked)
        .toBe(false);
});

test('reconciles silent native property changes and remains idle when properties do not change', async ({
    page,
}) => {
    await settings(page, 100);
    await installReadProbe(page);
    await page.evaluate(() => {
        const input = document.querySelector('#read-probe')!;

        input.value = 'Без события';
    });
    await expect
        .poll(async () => (await element(page, 'read-probe'))?.state.value)
        .toBe('Без события');
    await page.locator('#notifications').evaluate((input: HTMLInputElement) => {
        input.indeterminate = true;
    });
    await expect
        .poll(async () => (await element(page, 'notifications'))?.state.indeterminate)
        .toBe(true);
    const before = await count(page);
...
test('batches a burst once and publishes nothing for an unchanged recomputation', async ({
    page,
}) => {
    ...
    await expect
        .poll(async () => (await element(page, 'batch-probe'))?.tagName)
        .toBe('input');
    const before = await count(page);
    ...
    await expect
        .poll(
            async () => (await element(page, 'batch-probe'))?.attributes['data-sequence'],
        )
        .toBe('99');
    ...
    await expect.poll(async () => count(page)).toBe(before + 2);
    expect(await count(page, 'revision')).toBe(revision + 1);
...
test('refreshes geometry for window resize and external overlays', async ({page}) => {
    ...
    await expect
        .poll(async () => (await element(page, 'fixed-button'))?.hitTest)
        .toBe('hit');
    const x = (await element(page, 'resize-button'))!.rects[0].x;

    await page.setViewportSize({width: 1200, height: 1100});
    await expect
        .poll(async () => (await element(page, 'resize-button'))?.rects[0].x)
        .not.toBe(x);
    await page.evaluate(() => {
...
test('whole-document capture ignores inspector mutations, events and property polling', async ({
    ...
    await page.locator('app-observer-panel .inspector').evaluate((panel) => {
        panel.insertAdjacentHTML(
            'beforeend',
            '<div id="inspector-noise">Служебный текст</div>',
        );
    ...
    expect(
        Object.values(current.nodes).some(
            (node) => node.kind === 'element' && node.tagName === 'app-observer-panel',
        ),
    ).toBe(false);
});

test('stop cancels a queued capture, native polling and all event listeners; restart catches up', async ({
    page,
}) => {
    await settings(page, 100, 200);
    await installReadProbe(page);
    await page.evaluate(() => {
        document.querySelector('#read-probe')!.setAttribute('data-pending', 'yes');
        Array.from(document.querySelectorAll('button'))
            .find((button) => button.textContent?.trim() === 'Остановить')!
            .click();
    });
    await expect(page.getByTestId('observation-status')).toHaveText(
...
test('repeated starts replace the old polling interval and clear stops observation', async ({
    page,
}) => {
    await settings(page, 100);
    await installReadProbe(page);
...
test('DestroyRef releases polling and pending work when the inspector is unmounted', async ({
    page,
}) => {
    await settings(page, 100, 200);
    await installReadProbe(page);
    await page.evaluate(() => {
        document.querySelector('#read-probe')!.setAttribute('data-pending', 'yes');
        Array.from(document.querySelectorAll('button'))
            .find((button) => button.textContent?.trim() === 'Закрыть инспектор')!
            .click();
    });
    ...
    await expect
        .poll(async () => (await element(page, 'read-probe'))?.state.value)
        .toBe('После уничтожения');
});

test('reports a detached root and retains the last successful snapshot', async ({
    page,
}) => {
    await settings(page, 0);
    ...
    const before = await page.getByTestId('observed-page').evaluate((root) => {
        const published = JSON.parse(
            document.querySelector('[data-testid="snapshot-json"]')!.textContent!,
        );
...
test('tracks moving a control into and out of an excluded subtree', async ({page}) => {
    ...
    await page.getByTestId('observed-page').evaluate((root) => {
        root.insertAdjacentHTML(
            'beforeend',
            '<button id="moved-control">Перемещение</button><div id="excluded-area" data-training-observer-ignore></div>',
        );
    });
    await expect
        .poll(async () => (await element(page, 'moved-control'))?.tagName)
        .toBe('button');
    await page.evaluate(() => {
        document
            .querySelector('#excluded-area')!
            .append(document.querySelector('#moved-control')!);
    });
    await expect.poll(async () => element(page, 'moved-control')).toBeUndefined();
    await page.getByTestId('observed-page').evaluate((root) => {
        root.append(document.querySelector('#moved-control')!);
    });
    await expect
        .poll(async () => (await element(page, 'moved-control'))?.tagName)
        .toBe('button');
    await page
        .locator('#moved-control')
        .evaluate((button) => button.setAttribute('data-training-observer-ignore', ''));
    await expect.poll(async () => element(page, 'moved-control')).toBeUndefined();
    await page
        .locator('#moved-control')
        .evaluate((button) => button.removeAttribute('data-training-observer-ignore'));
    await expect
        .poll(async () => (await element(page, 'moved-control'))?.tagName)
        .toBe('button');
});

test('plain scroll does not rebuild or publish a snapshot, including with property polling enabled', async ({
    page,
}) => {
    await settings(page, 100);
    await page.getByTestId('observed-page').evaluate((root) => {
    ...
    await expect
        .poll(async () => (await element(page, 'passive-row'))?.label)
        .toBe('Строка');
    ...
    await page.locator('#passive-scroller').evaluate((scroller) => {
    ...
    expect(
        await page
            .locator('#passive-row')
            .evaluate((row) => row.getBoundingClientRect().y),
    ).toBe(oldY - 150);
    ...
    await expect
        .poll(async () => (await element(page, 'passive-row'))?.rects[0].y)
        .toBe(oldY - 150);
});

test('DOM changes caused by virtualized scrolling still update the snapshot with polling disabled', async ({
    page,
}) => {
    await settings(page, 0);
    await page.getByTestId('observed-page').evaluate((root) => {
        ...
        document.querySelector('#virtual-scroller')!.addEventListener(
            'scroll',
            () => {
                document.querySelector('#virtual-row')!.textContent = 'Строка 2';
            },
            {once: true},
        );
    });
    await expect
        .poll(async () => (await element(page, 'virtual-row'))?.label)
        .toBe('Строка 1');
    await page.locator('#virtual-scroller').evaluate((scroller) => {
        scroller.scrollTop = 150;
    });
    await expect
        .poll(async () => (await element(page, 'virtual-row'))?.label)
...
test('decorative hover on a real Taiga UI button does not rebuild the snapshot', async ({
    page,
}) => {
    await settings(page, 100);
    ...
    await expect
        ...
        .not.toBe(background);
    await page.waitForTimeout(700);
    const transitions = await button.evaluate(
        (element) =>
            (element as HTMLElement & {hoverTransitions: string[]}).hoverTransitions,
    );

    expect(transitions.length).toBeGreaterThan(0);
    expect(await count(page), `Hover transitions: ${transitions.join(', ')}`).toBe(
        before,
    );
    expect(await count(page, 'revision')).toBe(revision);
...
test('hover visibility transitions still refresh controls without DOM mutations', async ({
    page,
}) => {
    await settings(page, 0);
    await page.mouse.move(20, 200);
    ...
    await expect
        .poll(async () => (await element(page, 'hover-fade-control'))?.visible)
        .toBe(false);
...
test('menus inserted and removed on hover still update the page context', async ({
    page,
}) => {
    await settings(page, 0);
    await page.mouse.move(20, 200);
    await page.getByTestId('observed-page').evaluate((root) => {
        ...
        host.addEventListener('mouseenter', () =>
            host.insertAdjacentHTML(
                'beforeend',
                '<button id="hover-menu-action">Открыть</button>',
            ),
        );
        host.addEventListener('mouseleave', () =>
            document.querySelector('#hover-menu-action')?.remove(),
        );
        root.append(host);
    });
    await expect
        .poll(async () => (await element(page, 'hover-menu-trigger'))?.label)
        .toBe('Меню');
    await page.locator('#hover-menu-trigger').hover();
    await expect
        .poll(async () => (await element(page, 'hover-menu-action'))?.label)
        .toBe('Открыть');
    await page.mouse.move(20, 200);
    await expect.poll(async () => element(page, 'hover-menu-action')).toBeUndefined();
});

for (const wholeDocument of [false, true]) {
    test(`real wheel scrolling with Taiga UI page scrollbars does not trigger a capture (wholeDocument=${wholeDocument})`, async ({
        page,
    }) => {
        ...
        const thumb = page
            .locator('tui-scroll-controls [tuiScrollbar="vertical"]')
            .first();
        ...
        await expect
            ...
            .toBeGreaterThan(top);
        await expect.poll(async () => thumb.getAttribute('style')).not.toBe(style);
        await page.waitForTimeout(700);
        expect(await count(page)).toBe(before);
        expect(await count(page, 'revision')).toBe(revision);

        const left = await page.evaluate(() => window.scrollX);
        const horizontalThumb = page
            .locator('tui-scroll-controls [tuiScrollbar="horizontal"]')
            .first();
        ...
        await expect
            .poll(async () => page.evaluate(() => window.scrollX))
            .toBeGreaterThan(left);
        await expect
            .poll(async () => horizontalThumb.getAttribute('style'))
            .not.toBe(horizontalStyle);
        await page.waitForTimeout(700);
        expect(await count(page)).toBe(before);
        expect(await count(page, 'revision')).toBe(revision);
        expect(
            Object.values((await snapshot(page)).nodes).some(
                (node) =>
                    node.kind === 'element' &&
                    (node.tagName === 'tui-scroll-controls' ||
                        'tuiscrollbar' in node.attributes),
            ),
        ).toBe(false);
        ...
        await page.evaluate(() => {
            ...
            document
                ...
                .setAttribute('data-scroll-content', 'updated');
        });
        await expect
            .poll(
                async () =>
                    (await element(page, 'full-name'))?.attributes['data-scroll-content'],
            )
            .toBe('updated');
    });
}
