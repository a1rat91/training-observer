...
test('selection observed in a reopened popup survives closing and blur into the recording', async ({
    page,
}) => {
    ...
    await expect
        .poll(async () => {
            ...
            return state.controls?.find((c: any) => c.kind === 'combobox')?.choice
                .selection.status;
        })
        .toBe('observed');
...
test('retyping invalidates selection evidence but displayed text is still recorded after blur', async ({
    page,
}) => {
    await page.goto('/record');
    await page.getByRole('button', {name: 'Начать запись', exact: true}).click();
    ...
    await expect
        .poll(
            async () =>
                JSON.parse(
                    (await inspector.locator('pre').textContent()) ?? '{}',
                ).controls?.find((c: any) => c.kind === 'combobox')?.choice.selection
                    .status,
        )
        .toBe('observed');
    await page.locator('#employee').press('Escape');
    ...
    await page.getByRole('button', {name: 'Завершить запись', exact: true}).click();
    await expect(page.getByRole('region', {name: 'Журнал записи'})).toContainText(
        'Сотрудник — Анна Смирнова',
    );
});
