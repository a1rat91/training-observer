...
test('admin records blur and transitions, Stop includes the last edit and reload keeps the journal', async ({page}) => {
...
test('damaged storage is reported without crashing and a new recording replaces it', async ({page}) => {
    await page.addInitScript(() => localStorage.setItem('training-observer.state-recording.v1', '{"broken":true}'));
    await page.goto('/record');
