...
test('typed ComboBox text records after blur and can be published', async ({ page }) => {
    ...
    await page.getByText('Настроить тренировку', { exact: true }).click();
    await page.getByRole('button', { name: 'Создать ожидания из записи', exact: true }).click();
    await expect(page.getByRole('textbox', { name: 'Ожидание: Сотрудник', exact: true })).toHaveValue(
        'Анна Смирнова',
    );
    await expect(
...
test('wrong branch alerts once and returning to the expected screen allows completion', async ({ page }) => {
    ...
    await page.addInitScript(
        (value) => localStorage.setItem('training-observer.scenario.v1', JSON.stringify(value)),
        scenario,
    );
...
test('admin records initial choices and final radio state; learner must also preserve unchecked fields', async ({
    ...
    await expect(page.getByText('Ожидаем доступный экран.', { exact: true })).toHaveCount(0);
    await page.getByRole('button', { name: 'Далее', exact: true }).click();
