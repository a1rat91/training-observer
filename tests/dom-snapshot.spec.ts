...
test('detects occlusion and distinguishes rendered offscreen elements', async ({
    ...
    expect(byId(covered, 'covered-button')).toMatchObject({
        visible: true,
        inViewport: true,
        hitTest: 'covered',
        pointerActionable: false,
    });
    expect(byId(covered, 'offscreen-button')).toMatchObject({
        visible: true,
        inViewport: false,
        hitTest: 'not-tested',
    });
    await page.locator('#test-cover').evaluate((element) => element.remove());
    expect(byId(await capture(page), 'covered-button')).toMatchObject({
        hitTest: 'hit',
...
test('reports iframe and open shadow boundaries, ignores its own UI and excluded subtrees', async ({
    page,
}) => {
    await page.getByTestId('observed-page').evaluate((root) => {
        ...
        root.insertAdjacentHTML(
            'beforeend',
            '<iframe id="frame" srcdoc="<button>Frame</button>"></iframe><div data-training-observer-ignore><input id="excluded"></div><script>/* skipped */</script>',
        );
    });
    const snapshot = await capture(page);
