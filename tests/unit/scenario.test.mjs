...
const { matchControl } = await import('../../libs/training-runtime/src/lib/control-matcher.ts');
const { compileScenario } = await import('../../libs/training-recording/src/lib/scenario-compiler.ts');
const { parseScenario } = await import('../../libs/training-contracts/src/lib/scenario-codec.ts');
const { StateRecorder } = await import('../../libs/training-recording/src/lib/state-recorder.ts');
const { ScenarioRuntime } = await import('../../libs/training-runtime/src/lib/scenario-runtime.ts');
...
test('wrong transition cannot bypass missing fields; return, correction and last blur at navigation work', () => {
    ...
    r.update(screen('A', [a, b]), {});
    assert.equal(
        r.update(screen('B'), { a: control('a', 'Имя', 'Анна'), b: control('b', 'Город', 'Казань') }).status,
        'complete',
    );
});
test('ambiguous, missing and unknown choice block assessment without blaming the learner', () => {
    ...
    assert.deepEqual(ambiguous.feedback, []);
    assert.equal(r.update({ ...screen('A'), status: 'loading' }, {}).status, 'waiting');
    assert.equal(
        r.update(screen('A', [a, b]), { a: { ...a, choice: { selection: { status: 'unknown' } } } }).status,
        'blocked',
    );
});
test('correct value changed to wrong is revoked; optional expectations do not block', () => {
    const s = {
        ...
        steps: [
            {
                ...scenario.steps[0],
                fields: [field('Имя', 'Анна'), { ...field('Нет поля', ''), optional: true }],
            },
        ],
    };
...
test('ComboBox compares displayed text only after confirmation, including clearing and correction', () => {
    ...
    assert.deepEqual(wrong.feedback, ['Ошибка Сотрудник']);
    assert.equal(r.update(screen('A', [c]), { c: combo('Анна') }).status, 'complete');
    assert.equal(
        r.update(screen('A', [c]), { c: { ...combo('Анна'), state: { redacted: true } } }).status,
        'blocked',
    );
});
...
test('new screen checkbox defaults stay quiet; changes report errors and unmet fields still block transition', () => {
    ...
    assert.deepEqual(entered.feedback, []);
    assert.equal(entered.completedFields, 0);
...
test('late appearing immediate controls get a quiet baseline without accepting wrong defaults', () => {
    ...
    const r = new ScenarioRuntime({
        ...scenario,
        steps: [
            { ...scenario.steps[0], fields: [{ ...field('Согласие', true), descriptor: c.locatorHints }] },
        ],
    });
    assert.equal(r.update(screen('A'), {}).status, 'blocked');
    const first = r.update(screen('A', [c]), {});
    assert.equal(first.status, 'active');
    assert.equal(first.completedFields, 0);
...
test('prefilled answers are accepted on entry and after returning from a wrong screen', () => {
    ...
    assert.equal(first.completedFields, 2);
    assert.deepEqual(first.feedback, []);
    assert.deepEqual(r.update(screen('WRONG'), {}).feedback, ['Неверный переход']);
    const returned = r.update(
        screen('A', [control('new-a', 'Имя', 'Анна'), control('new-b', 'Город', 'Казань')]),
        {},
    );
    assert.equal(returned.completedFields, 2);
    assert.deepEqual(returned.feedback, []);
...
test('wrong prefilled text stays quiet and cannot advance; later edits require blur', () => {
    const r = new ScenarioRuntime({
        ...scenario,
        steps: [{ ...scenario.steps[0], fields: [field('Имя', 'Анна')] }, scenario.steps[1]],
    });
    ...
    assert.equal(blurred.completedFields, 0);
    assert.deepEqual(blurred.feedback, ['Ошибка Имя']);
...
test('a prefilled amount on a later screen is evaluated immediately; edits wait for blur', () => {
    ...
    for (const value of ['1500', '2000']) {
        ...
        assert.equal(committed.completedFields, 0);
        assert.deepEqual(committed.feedback, ['Ошибка Сумма']);
    }
});
...
test('recorded checkbox/radio picture compiles last states and false remains a required answer', () => {
    const choice = (id, kind, checked) =>
        control(id, id, '', { kind, state: { checked }, locatorHints: { ...descriptor(id), kind } });
    const start = [
    ...
    assert.deepEqual(
        model.steps[0].fields.map((f) => f.expected),
        [false, true, false],
    );
    ...
    assert.equal(wrong.completedFields, 2);
    assert.equal(wrong.feedback.length, 1);
});
