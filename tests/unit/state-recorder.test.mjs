...
import { StateRecorder } from '../../libs/training-recording/src/lib/state-recorder.ts';
import { parseStateRecording } from '../../libs/training-contracts/src/lib/recording-codec.ts';
const field = (id, value, extra = {}) => ({
    id,
    targetNodeId: id,
    kind: 'textbox',
    state: { value, redacted: false },
    locatorHints: { kind: 'textbox', label: id, role: 'textbox', tagName: 'input', context: [] },
    ...extra,
});
...
test('attributes departure blur to A before recording B, and counts return visits', () => {
    ...
    assert.deepEqual(
        events.map((e) => [e.kind, e.screenKey, e.visit]),
        [
            ['screen', 'A', 1],
            ['value', 'A', 1],
            ['screen', 'B', 2],
...
test('checkbox records initial state and changes, but not disabled-only updates', () => {
    ...
    const a = field('check', undefined, {
        kind: 'checkbox',
        state: { checked: true },
        locatorHints: { kind: 'checkbox', label: 'Check', tagName: 'input', role: 'checkbox', context: [] },
    });
    r.start(screen('A', [a]), {});
