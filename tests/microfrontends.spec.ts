...
test('a real Taiga dropdown belongs only to the employment area', async ({ page }) => {
    ...
    await page.locator('#mf-department').click();
    await expect
        .poll(async () => (await area(page, 'employment')).logicalControls[0].choice?.popup.options.length)
        .toBe(3);
    ...
    await expect
        .poll(async () => (await area(page, 'employment')).logicalControls[0].choice?.displayValue)
        .toBe('Поддержка');
    await settle(page);
...
test('moving a regular control updates both owners', async ({ page }) => {
    await page.evaluate(() =>
        document.getElementById('mf-address')!.append(document.getElementById('mf-comment')!),
    );
    await expect
        .poll(async () =>
...
test('external labels and common ancestor styles invalidate dependent areas', async ({ page }) => {
    ...
    await expect
        .poll(async () =>
            (await areas(page)).every((item) => item.logicalControls.every((control) => !control.visible)),
        )
        .toBe(true);
});
...
test('shared stylesheet insertion and delayed load refresh all areas', async ({ page }) => {
    ...
    await expect
        .poll(async () => (await areas(page)).every((item) => item.scanCount > before[item.id]))
        .toBe(true);
    // Complete loading after the insertion batch, without any further DOM mutation.
    await settle(page);
    expect((await areas(page)).every((item) => item.logicalControls.some((control) => control.visible))).toBe(
        true,
    );
    releaseStyles();
    await expect
