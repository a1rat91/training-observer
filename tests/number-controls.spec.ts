...
test('real Taiga numbers preserve labels, exact formatting and big integers', async ({page}) => {
    expect(await control(page, 'amount')).toMatchObject({kind: 'number', source: 'taiga-ui', label: 'Сумма',
        state: {required: true}, locatorHints: {role: 'textbox', inputType: 'text'}});
    expect((await control(page, 'amount'))?.state.value).toContain('₽');
    await expectObservedValue(page, 'international-amount');
    expect(await control(page, 'international-amount')).toMatchObject({kind: 'number', source: 'taiga-ui',
    ...
    expect(String((await control(page, 'large-number'))?.state.value).replace(/\D/g, ''))
        .toBe('900719925474099312345');
    expect(await control(page, 'native-quantity')).toMatchObject({kind: 'number', source: 'native',
        label: 'Количество (HTML)', state: {value: '2'}, locatorHints: {role: 'spinbutton', inputType: 'number'}});
});
...
test('typing, step buttons, keyboard and cleaner update the same numeric control', async ({page}) => {
    ...
    const cleaner = amount.memberNodeIds.map((id) => dom.nodes[id])
    ...
    const buttons = (await controls(page)).filter((item) => item.kind === 'button' && ['+', '-'].includes(item.label));
    expect(buttons).toHaveLength(2);
    for (const button of buttons) expect(amount.memberNodeIds).not.toContain(button.targetNodeId);
});

test('numeric boundaries update step availability; readonly and disabled remain observable', async ({page}) => {
    await page.getByText('InputNumber', {exact: true}).click();
    const input = page.locator('#amount');
    await input.fill('2000');
    await expectObservedValue(page, 'amount');
    await expect.poll(async () => (await controls(page)).find((item) => item.label === '+')?.state.disabled).toBe(true);
    await input.fill('0');
    await expect.poll(async () => (await controls(page)).find((item) => item.label === '-')?.state.disabled).toBe(true);
    await input.evaluate((element: HTMLInputElement) => { element.readOnly = true; });
...
test('native validity and silent numeric value writes are reconciled', async ({page}) => {
    ...
    await page.locator('#native-quantity').evaluate((input: HTMLInputElement) => { input.value = '8'; });
    ...
    await page.locator('#amount').evaluate((input: HTMLInputElement) => { input.value = '1 999,5 ₽'; });
...
test('numeric keyboards are not enough to classify text fields; conflicting native roles stay unsupported', async ({page}) => {
...
test('a numeric field keeps its explicit popup relationship without claiming selection semantics', async ({page}) => {
    await page.evaluate(() => {
        document.body.insertAdjacentHTML('beforeend', '<div id="number-popup" role="dialog">Подсказка к сумме</div>');
    ...
    expect(await control(page, 'amount')).toMatchObject({kind: 'number', popup: {
        relation: 'aria-controls', referencedIds: ['number-popup'], text: 'Подсказка к сумме', options: [],
    }});
