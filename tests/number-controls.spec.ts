...
test('real Taiga numbers preserve labels, exact formatting and big integers', async ({
    ...
    expect((await control(page, 'international-amount'))?.state.value).toContain(
        '1,234.5',
    );
...
test('typing, step buttons, keyboard and cleaner update the same numeric control', async ({
    page,
}) => {
    ...
    const dom: DomSnapshot = JSON.parse(
        (await page.getByTestId('snapshot-json').textContent()) ?? '{}',
    );
    ...
    for (const button of buttons) {
        expect(amount.memberNodeIds).not.toContain(button.targetNodeId);
    }
});
...
test('numeric boundaries update step availability; readonly and disabled remain observable', async ({
    ...
    await expect
        .poll(
            async () =>
                (await controls(page)).find((item) => item.label === '+')?.state.disabled,
        )
        .toBe(true);
    await input.fill('0');
    await expect
    ...
    await expect
        .poll(async () => (await control(page, 'amount'))?.state.readOnly)
        .toBe(true);
    await input.evaluate((element: HTMLInputElement) => {
        element.disabled = true;
    });
    await expect
        .poll(async () => (await control(page, 'amount'))?.state.disabled)
        .toBe(true);
...
test('native validity and silent numeric value writes are reconciled', async ({page}) => {
    ...
    await expect
        .poll(async () => (await control(page, 'native-quantity'))?.state.invalid)
        .toBe(true);
    await page.locator('#native-quantity').evaluate((input: HTMLInputElement) => {
        input.value = '8';
    });
    await expect
        .poll(async () => (await control(page, 'native-quantity'))?.state.value)
        .toBe('8');
    ...
    await expect
        .poll(async () => (await control(page, 'amount'))?.state.value)
        .toBe('1 999,5 ₽');
...
test('numeric keyboards are not enough to classify text fields; conflicting native roles stay unsupported', async ({
    ...
    await expect
        .poll(async () => (await control(page, 'account-text'))?.state.value)
        .toBe('00123');
...
test('a numeric field keeps its explicit popup relationship without claiming selection semantics', async ({
    ...
    await expect
        .poll(async () => (await control(page, 'amount'))?.popup?.status)
        .toBe('open');
    expect(await control(page, 'amount')).toMatchObject({
        kind: 'number',
        popup: {
