...
test('Taiga select tracks absent, opened, selected, closed and recreated portal outside the form', async ({
    page,
}) => {
    const initial = await control(page, 'department');

    expect(initial.choice).toMatchObject({
        displayValue: 'Разработка',
        popup: {status: 'closed', options: [], rootNodeIds: []},
    });
    expect(await page.locator('tui-dropdown').count()).toBe(0);
    await page.locator('#department').click();
    await expect
        .poll(async () =>
            (await control(page, 'department')).choice?.popup.options.map(
                (option) => option.label,
            ),
        )
    ...
    expect(await page.getByTestId('observed-page').locator('tui-dropdown').count()).toBe(
        0,
    );
    expect(
        (await controls(page)).filter((item) => item.kind === 'checkbox'),
    ).toHaveLength(1);

    await page.getByRole('option', {name: 'Поддержка', exact: true}).click();
    await expect
        .poll(async () => (await control(page, 'department')).choice?.displayValue)
        .toBe('Поддержка');
    await expect
        .poll(async () => (await control(page, 'department')).choice?.popup.status)
        .toBe('closed');
    ...
    await expect
        .poll(
            async () =>
                (await control(page, 'department')).choice?.popup.options.find(
                    (option) => option.selected,
                )?.label,
        )
        .toBe('Поддержка');
    expect((await control(page, 'department')).id).toBe(initial.id);
    expect((await control(page, 'department')).choice?.popup.rootNodeIds).not.toEqual(
        opened.choice!.popup.rootNodeIds,
    );
});

test('Taiga combobox exposes query, asynchronous loading, rendered results and selection evidence', async ({
    page,
}) => {
    await page.locator('#employee').fill('Анна');
    await expect
        .poll(async () => (await control(page, 'employee')).choice?.popup.busy, {
            intervals: [25],
        })
        .toBe(true);
    expect((await control(page, 'employee')).choice).toMatchObject({
        displayValue: 'Анна',
    ...
    await expect
        .poll(async () =>
            (await control(page, 'employee')).choice?.popup.options.map(
                (option) => option.label,
            ),
        )
    ...
    await expect
        .poll(async () => (await control(page, 'employee')).choice?.displayValue)
        .toBe('Анна Смирнова');
    await expect
        .poll(async () => (await control(page, 'employee')).choice?.popup.status)
        .toBe('closed');
    ...
    await expect
        .poll(async () => (await control(page, 'employee')).choice?.selection.labels)
        .toEqual(['Анна Смирнова']);
});

test('latest search results, empty results and Escape update the same combobox', async ({
    page,
}) => {
...
test('an unrelated listbox is not attributed to the active field, even with whole-document capture', async ({
    page,
}) => {
    await page.evaluate(() =>
    ...
    await expect
        .poll(
            async () => (await control(page, 'department')).choice?.popup.options.length,
        )
        .toBe(3);
    expect(
    ...
    await expect
        ...
        .toBe(3);
    expect(
        (await control(page, 'department')).choice?.popup.options.some(
            (option) => option.label === 'Чужой вариант',
        ),
    ).toBe(false);
    expect((await snapshot(page)).relatedRootIds).toEqual([]);
});

test('missing explicit popup link remains unresolved rather than choosing another listbox', async ({
    page,
}) => {
    await page
    ...
    await page.evaluate(() =>
        document.body.insertAdjacentHTML(
            'beforeend',
            '<div role="listbox"><button role="option">Не угадывать</button></div>',
        ),
    );
    await expect
        .poll(async () => (await control(page, 'unlinked-choice'))?.choice?.popup.status)
        .toBe('unresolved');
    expect((await control(page, 'unlinked-choice')).choice?.popup.options).toEqual([]);
});

test('native select also exposes selected labels, values and disabled options', async ({
    page,
}) => {
    ...
    await expect
        .poll(async () => (await control(page, 'native-choice'))?.choice?.displayValue)
        .toBe('Второй');
...
test('keyboard option selection is reflected without a mouse click on the option', async ({
    page,
}) => {
    await page.locator('#department').click();
    await expect
...
test('portal capture respects the global node limit and explicit exclusions', async ({
    page,
}) => {
    await page
        .getByTestId('observed-page')
    ...
    await page.evaluate(() => {
        ...
        list.innerHTML = Array.from(
            {length: 100},
            (_, i) => `<button role="option">Вариант ${i}</button>`,
        ).join('');
    ...
    await expect
        .poll(
            async () =>
                (await control(page, 'limited-choice'))?.choice?.popup.options.length,
        )
        .toBe(100);
    await page.locator('.settings').evaluate((details: HTMLDetailsElement) => {
        details.open = true;
    ...
    for (const node of Object.values(limited.nodes)) {
        if (node.kind === 'element') {
            for (const id of node.children) {
                expect(limited.nodes[id]?.parentId).toBe(node.id);
            }
        }
    }

    await page
        .locator('#limited-list')
        .evaluate((list) => list.setAttribute('data-training-observer-ignore', ''));
    await expect
        .poll(async () => (await control(page, 'limited-choice')).choice?.popup.status)
        .toBe('unresolved');
