...
test('Taiga select tracks absent, opened, selected, closed and recreated portal outside the form', async ({
    ...
    await expect
        .poll(async () =>
            (await control(page, 'department')).choice?.popup.options.map((option) => option.label),
        )
    ...
    await expect
        .poll(
            async () =>
                (await control(page, 'department')).choice?.popup.options.find((option) => option.selected)
                    ?.label,
        )
        .toBe('Поддержка');
...
test('Taiga combobox exposes query, asynchronous loading, rendered results and selection evidence', async ({
    ...
    await expect
        .poll(async () => (await control(page, 'employee')).choice?.popup.busy, { intervals: [25] })
        .toBe(true);
    ...
    await expect
        .poll(async () =>
            (await control(page, 'employee')).choice?.popup.options.map((option) => option.label),
        )
...
test('portal capture respects the global node limit and explicit exclusions', async ({ page }) => {
    ...
    await expect
        .poll(async () => (await control(page, 'limited-choice'))?.choice?.popup.options.length)
        .toBe(100);
    await page.locator('.settings').evaluate((details: HTMLDetailsElement) => {
        details.open = true;
    });
    ...
    for (const node of Object.values(limited.nodes)) {
        if (node.kind === 'element')
            for (const id of node.children) expect(limited.nodes[id]?.parentId).toBe(node.id);
    }
    await page
