...
const hint={kind:'textbox',label:'Фамилия',id:'surname',tagName:'input',role:'textbox',context:[]};
const recording={kind:'training-state-recording',version:1,complete:true,events:[
    ...
    {sequence:2,kind:'value',visit:1,screenKey:'application-profile',field:hint,value:'Первое'},
    {sequence:3,kind:'value',visit:1,screenKey:'application-profile',field:hint,value:'Второе'},
    {sequence:4,kind:'screen',visit:2,screenKey:'application-details'},
]};
async function seed(page: import('@playwright/test').Page) {
    await page.goto('/record');
    await page.evaluate(value=>localStorage.setItem('training-observer.state-recording.v1',JSON.stringify(value)),recording);
    await page.reload();
...
test('delete and undo rebuild source values while preserving authored feedback and published scenario',async({page})=>{
    await seed(page);
...
test('deleting a screen boundary blocks compilation; undo repairs it; edited journal survives reload',async({page})=>{
    await seed(page);
    ...
    await page.getByRole('button',{name:'Создать ожидания из записи',exact:true}).click();
    await expect(page.getByRole('button',{name:'Сохранить сценарий для ученика',exact:true})).toBeDisabled();
    await page.getByRole('button',{name:'Отменить удаление',exact:true}).click();
    await page.getByRole('button',{name:'Создать ожидания из записи',exact:true}).click();
