...
Все модели readonly и сериализуемы. У них нет методов, subscriptions или DOM Element. ID узла и контрола относится к
сеансу, а не к постоянной идентичности интерфейса. ControlLocatorHints содержит признаки поиска; текущий ответ и
координаты в него не входят. `displayValue` ComboBox и `selection` — разные факты. Unknown не означает пустое значение.
