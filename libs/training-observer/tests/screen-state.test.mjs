...
import {readScreenState} from '../src/lib/services/screen-state-reader.ts';
import {ScreenVisitTracker} from '../src/lib/services/screen-visit-tracker.ts';

const options = {root: {tagName: 'section', attribute: {name: 'aria-label', value: 'Editor'}}, identity: {kind: 'attribute', name: 'id'}, loading: {name: 'aria-busy', value: 'true'}};
const element = (id, parentId, attributes = {}, tagName = 'div') => ({id, parentId, attributes, tagName, kind: 'element', visible: true, children: []});
function fixture() {
    return {schemaVersion: 1, rootId: 'body', capturedAt: '', durationMs: 0, interactiveIds: ['a', 'outside'], stats: {truncated: false}, nodes: {
...
const controls = [{id: 'control:a', targetNodeId: 'a'}, {id: 'control:outside', targetNodeId: 'outside'}];
...
test('supports a different application tag and existing identity attribute without domain knowledge', () => {
    ...
    assert.equal(read(snapshot, {root: {tagName: 'account-panel'}, identity: {kind: 'attribute', name: 'name'}}).key, 'account');
});
...
test('direct text identity excludes descendant fields and their values', () => {
    ...
    snapshot.nodes.heading = {id: 'heading', parentId: 'screen', kind: 'text', text: ' Stable key ', visible: true};
...
test('refuses an unspecified root or empty attribute name', () => {
    ...
    assert.throws(() => read(fixture(), {...options, identity: {kind: 'attribute', name: ''}}), /explicit/);
});
