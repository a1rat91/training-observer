 ...
 */
import type { ScreenState, ScreenVisit } from '@training-observer/core/models';

export class ScreenVisitTracker {
    private current: ScreenVisit | null = null;

    update(state: ScreenState): ScreenVisit | null {
        if (state.status !== ScreenStatus.Ready || !state.key || !state.rootNodeId) return this.current;
        this.current = {
            key: state.key,
