...
import {type DomControlState, type InteractionReason} from '../models/dom-snapshot';
...
const INTERACTIVE_ROLES = new Set([
    ...
    'menuitemradio', 'scrollbar', 'treeitem',
]);

@Injectable({providedIn: 'root'})
export class DomElementAnalyzer {
    interactionReasons(element: Element, geometry: DomGeometry, cursorHeuristics: boolean): InteractionReason[] {
        const reasons: InteractionReason[] = [];

        if (element.matches('button,input:not([type="hidden"]),select,textarea,a[href],area[href],summary,label')) {
            reasons.push('native');
        }
        ...
        // Optional weak signal. Do not label every descendant inheriting the cursor as a new action.
        if (cursorHeuristics && geometry.style(element).cursor === 'pointer' &&
    ...
    attributes(element: Element): Record<string, string> {
        ...
        for (const name of element.getAttributeNames()) {
            // Angular debug attributes and event-handler source are not page state.
    ...
    state(element: Element): DomControlState {
        ...
        const checked = element.matches('input[type="checkbox"],input[type="radio"]') ? control.checked :
            this.ariaBoolean(element, 'aria-checked');
        const indeterminate = element.matches('input[type="checkbox"]') ? control.indeterminate :
            element.getAttribute('aria-checked') === 'mixed' ? true : undefined;
        const selected = element.matches('option') ? (element as HTMLOptionElement).selected :
            this.ariaBoolean(element, 'aria-selected');
        const expanded = element.matches('details') ? (element as HTMLDetailsElement).open :
    ...
    private editable(element: Element): boolean {
        // Only the editing host is an action, not every formatted descendant.
