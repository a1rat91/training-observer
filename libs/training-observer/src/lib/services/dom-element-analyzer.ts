...
export class DomElementAnalyzer {
    ...
    public label(element: Element): string {
        const labelledBy = element
            ...
            .map(
                (id) =>
                    element.ownerDocument.querySelector(`#${CSS.escape(id)}`)
                        ?.textContent ?? '',
            )
            .join(' ');
        ...
        if (element.localName === 'option') {
            text = (element as HTMLOptionElement).label;
        } else if (element.matches('button,a,summary,[role="button"],[role="option"]')) {
    ...
    public state(element: Element): DomControlState {
        ...
        const state: DomControlState = {
            ...
            readOnly:
                control.readOnly || element.getAttribute('aria-readonly') === 'true',
