...
/** Reads one captured graph. Never resolves references against the live document. */
export class SnapshotReader {
    ...
    constructor(readonly snapshot: DomSnapshot) {
        this.elements = Object.values(snapshot.nodes)
    ...
    /** Preserves the existing context-label policy: the last duplicate HTML id wins. */
    referencedText(ids: string): string {
        return ids.trim().split(/\s+/)
            ...
            .map((node) => this.text(node.id)).join(' ');
    }

    legendText(fieldset: DomElementSnapshot): string {
        const legend = fieldset.children.map((id) => this.snapshot.nodes[id])
    ...
    /** Keeps traversal order and stops at other controls, including all their descendants. */
    members(
