import {type DomNodeId} from '../models/dom-snapshot';
import {DomGeometry} from './dom-geometry';
import {OBSERVER_UI_ATTRIBUTE} from './dom-observer-ui';
...
/** One presentation layer and one animation loop, independent of DOM capture. */
export class DomHighlightLayer {
    ...
    constructor(private readonly document: Document, private readonly targets: readonly HighlightTarget[]) {
        ...
        this.host.style.cssText = 'all:initial!important;position:fixed!important;inset:0!important;pointer-events:none!important;z-index:2147483647!important;overflow:hidden!important;contain:strict!important;';
    ...
    private isRendered(element: Element): boolean {
        // Closed details can expose stale client rects for descendants in skipped content.
    ...
    private update = (): void => {
        // Geometry caches belong to one frame; never reuse snapshot rectangles on scroll.
        const geometry = new DomGeometry(this.view);
        for (const target of this.targets) {
            const element = target.element.deref();
            const rects = element?.isConnected && element.ownerDocument === this.document && this.isRendered(element) && geometry.visible(element)
                ? geometry.rects(element).filter((rect) => rect.bottom > 0 && rect.right > 0 &&
                    rect.top < this.view.innerHeight && rect.left < this.view.innerWidth) : [];
