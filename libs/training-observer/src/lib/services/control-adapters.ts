...
export function findControlCandidates(reader: SnapshotReader): readonly ControlCandidate[] {
    ...
    for (const target of reader.elements) {
        ...
        if (field && 'multi' in field.attributes && (match.kind === 'textbox' || match.kind === 'number')) continue;
    ...
    return candidates.filter(({target, ancestors}) =>
...
export function hasTaigaInputPopup(control: ControlCandidate): boolean {
    return control.source === 'taiga-ui' && (control.kind === 'textbox' || control.kind === 'number') &&
...
function isTaigaFiller(node: DomElementSnapshot): boolean {
    return node.tagName === 'input' && node.state.disabled && node.attributes['aria-hidden'] === 'true' &&
...
function nativeKind(node: DomElementSnapshot): ControlKind | null {
    ...
    const textInput = node.tagName === 'input' && ['text', 'search', 'email', 'tel', 'url', 'password'].includes(type);
...
function matchTaigaControl(
    ...
    if ((native === 'button' && ('tuibutton' in attributes || 'tuiiconbutton' in attributes)) ||
