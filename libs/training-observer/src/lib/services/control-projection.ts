import {type ControlKind, type ControlLocatorHints, type ControlSnapshot} from '../models/control-snapshot';
import {type DomControlState, type DomElementSnapshot, type DomNodeId} from '../models/dom-snapshot';
...
export function projectControl(
    ...
    return {
        ...
        rects: target.rects,
        choice: kind === 'select' || kind === 'combobox'
            ? buildChoiceSnapshot(reader.snapshot, target, host, kind) : undefined,
...
function controlLabel(
    ...
    if (control.kind === 'button' && typeof control.target.state.value === 'string') return control.target.state.value;
...
function locatorHints(reader: SnapshotReader, control: ControlCandidate, label: string): ControlLocatorHints {
    ...
    return {
        ...
        context: ancestors.filter(isContextElement).reverse().map((element) => ({
...
function isContextElement(element: DomElementSnapshot): boolean {
    return ['form', 'fieldset', 'section', 'dialog'].includes(element.tagName) ||
