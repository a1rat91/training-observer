/** Обход DOM в рамках одного capture. Сохраняет узлы и связанные popup, применяет исключения и лимиты, не изменяет исходную страницу. */
import {
...
export class DomCapture {
    ...
    private captureElement(
        ...
        const interactionReasons = this.analyzer.interactionReasons(
            element,
            this.geometry,
        ...
        const data: DomElementSnapshot = {
            ...
            path,
            attributes: this.analyzer.attributes(element),
            label: this.analyzer.label(element),
            children,
            rects: this.geometry.serializeRects(element),
            visible,
            inViewport,
            hitTest,
            interactive,
