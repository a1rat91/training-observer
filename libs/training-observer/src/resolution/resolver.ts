/**
 * ElementResolver — ищет сохранённую цель в текущем DOM и объясняет принятие либо отказ.
 * Алгоритм: проверяет формат и pathname, собирает видимые controls, вычисляет identity evidence,
 * ограничивает кандидатов семантическим контекстом и проверяет locators. Лучший кандидат принимается
 * только при достаточном score, отрыве от конкурента и допустимом состоянии; иначе ambiguous/broken.
 * CSS не заменяет identity proof. Поиск синхронный, DOM не изменяет и ссылок между вызовами не сохраняет.
 */
import {
