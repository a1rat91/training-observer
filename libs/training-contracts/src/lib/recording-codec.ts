...
import type { StateRecording, RecordedEvent } from './recording.models';
...
export function parseStateRecording(json: string): StateRecording {
    if (json.length > MAX_DOCUMENT_LENGTH) throw new Error('Запись слишком большая.');
    ...
    return { kind: 'training-state-recording', version: 1, complete: data['complete'], events };
}

function isRecordedEvent(event: unknown, sequence: number, complete: boolean): event is RecordedEvent {
    if (
        !isObject(event) ||
        !hasOnlyKeys(event, ['sequence', 'visit', 'screenKey', 'kind', 'field', 'value', 'reason']) ||
    ...
    switch (event['kind']) {
        ...
        case RecordingEventKind.Screen:
            return (
                event['field'] === undefined && event['value'] === undefined && event['reason'] === undefined
            );
        case RecordingEventKind.Unavailable:
            return !complete && event['value'] === undefined && typeof event['reason'] === 'string';
        default:
            return false;
    }
}
