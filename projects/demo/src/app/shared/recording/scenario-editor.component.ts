        ...
        <section aria-label="Редактор сценария">
            ...
            @if (draft(); as scenario) {
                @for (step of scenario.steps; track $index; let si = $index) {
                    <fieldset><legend>Экран {{ si + 1 }} · {{ step.key }}</legend>
                ...
                <button tuiButton size="s" type="button" (click)="publish()">Сохранить сценарий для ученика</button>
                @if (published()) { <p>Сценарий сохранён. <a routerLink="/learn">Перейти к тренировке</a></p> }
            }
...
export class ScenarioEditorComponent {
    ...
    constructor() { afterNextRender(() => { this.store.load(); this.draft.set(this.store.saved()); this.fromSaved.set(!!this.store.saved()); }); }
    protected compile(): void {
        try { this.draft.set(compileScenario(this.recording())); this.fromSaved.set(false); this.error.set(''); this.published.set(false); }
        catch (e) { this.error.set(e instanceof Error ? e.message : 'Не удалось создать сценарий.'); }
    }
    ...
    protected publish(): void {
        ...
        if (draft) { this.store.save(draft); this.published.set(!this.store.error()); }
    }
}
