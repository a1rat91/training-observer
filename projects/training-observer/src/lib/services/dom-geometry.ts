import {type DomRectSnapshot, type HitTestResult} from '../models/dom-snapshot';

/** Per-capture cache: never reuse geometry or live computed styles across snapshots. */
export class DomGeometry {
    ...
    private stylesAllowVisibility(element: Element): boolean {
        ...
        // A child can override visibility:hidden, but cannot override an ancestor's opacity:0.
    ...
    private clipToViewport(rect: DOMRect): {left: number; top: number; right: number; bottom: number} | null {
